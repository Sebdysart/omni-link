...
export function detectConventions(
  ...
  const errorHandling = detectErrorHandling(sourceSnippets ?? []);
...
function detectFileOrganization(files: FileInfo[]): string {
  ...
  if (featureDirs > 0 && featureDirs >= layerDirCount) {
...
function detectErrorHandling(sourceSnippets: string[]): string {
  ...
  if (tryCatch > promiseCatch && tryCatch > resultType) return 'try-catch';
  if (promiseCatch > tryCatch && promiseCatch > resultType) return 'promise-catch';
  if (resultType > tryCatch && resultType > promiseCatch) return 'result-type';
  if (tryCatch > 0) return 'try-catch';
  if (promiseCatch > 0) return 'promise-catch';
  if (resultType > 0) return 'result-type';
...
function detectPatterns(files: FileInfo[], language: string): string[] {
  ...
  if (language === 'typescript' || language === 'javascript') {
    const hasTrpc = files.some((f) => f.path.includes('router') || f.path.includes('trpc'));
